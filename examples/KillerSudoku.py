...
def get_model(N, clues, cages):
    ...
    sudoku = Model(
        ...
        [AllDiff([grid.flat[int(cell)] for cell in cage.split()[1:]])
            for cage in open(cages)],

        # [AllDiff(cells) for cage in open(cages) for cells in grid.flat
        # if cells.position in cage.split()[1:]],

        # Contents of cages must add up to the cage's number.
        [Sum([grid.flat[int(cell)] for cell in cage.split()[1:]])
            == int(cage.split()[0]) for cage in open(cages)])
...
if __name__ == '__main__':
    default = {
        'N': 3, 'solver': 'Mistral', 'verbose': 1,
        'hints': 'data/057-KillerSudokuHints.txt',
        'cages': 'data/057-KillerSudokuCages.txt'}
    param = input(default)
    solve(param)
