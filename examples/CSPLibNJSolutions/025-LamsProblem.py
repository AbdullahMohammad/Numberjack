...
# The goal is to put 11 zeros in each row in such a way that each
...
def get_model(N):
    ...
    model = Model(
        # Each row must contain 11 zeroes (the rest 100 are 1s).
        [Sum(row) == 100 for row in matrix.row],

        # Each column must contain 11 zeroes (the rest 100 are 1s).
